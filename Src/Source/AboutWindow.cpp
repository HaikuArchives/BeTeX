...
void AboutView::Draw(BRect updateRect)
{
	...
	StrokeLine(BPoint(updateRect.right,updateRect.top+1),BPoint(updateRect.right,updateRect.bottom));				
}

void AboutView::ConvertRGB32ToGreyRGB32(BBitmap* b)
{
				...
				:	BWindow(frame,"About BeTeX",B_FLOATING_WINDOW,B_NOT_ZOOMABLE|B_NOT_RESIZABLE)
{
	...
	int bheight = 30;
	int bpad = 10;
	BRect btnRect(frame.right - (2*bpad) - bwidth,frame.bottom - (2*bpad) - bheight,frame.right - (2*bpad),frame.bottom - (2*bpad));
	m_quit = new BButton(btnRect,"quitbtn","OK",new BMessage(AboutMessages::K_ABOUT_WINDOW_QUIT));
	m_quit->MakeDefault(true);
	m_gradient->AddChild(m_quit);
	
	bwidth = 100;
	btnRect = BRect(frame.right - (2*bpad) - bwidth,frame.bottom - (2*bpad) - bheight,frame.right - (2*bpad),frame.bottom - (2*bpad));
	btnRect.OffsetBy(-(50 + (2*bpad)),0);
	
	m_homePage = new BButton(btnRect,"homepagebtn","BeTeX Homepage",new BMessage(AboutMessages::K_GOTO_BETEX_HOMEPAGE));
	m_gradient->AddChild(m_homePage);
	
	btnRect.OffsetBy(-(80 + (2*bpad)),0);
	btnRect.right = btnRect.left+80;
	m_beBits = new BButton(btnRect,"fBeBitsbtn","BeBits Page",new BMessage(AboutMessages::K_GOTO_BETEX_BEBITSPAGE));
	m_gradient->AddChild(m_beBits);
	
	btnRect.OffsetBy(-(60 + (2*bpad)),0);
	btnRect.right = btnRect.left+60;
	m_donate = new BButton(btnRect,"fDonatebtn","Donate",new BMessage(AboutMessages::K_GOTO_BETEX_DONATEPAGE));

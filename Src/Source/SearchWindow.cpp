					...
					m_msgr(messenger)
{
	...
	*/
	...
	//construct buttons
	BButton *find = new BButton(buttonRect,"find","Find",new BMessage(SearchWindowConstants::K_SEARCH_WINDOW_FIND));
	parent->AddChild(find);
	find->MakeDefault(true);
	buttonRect.OffsetBy(0.0f, buttonHeight + 10.0f);
	
	BButton *replace = new BButton(buttonRect,"replace","Replace",new BMessage(SearchWindowConstants::K_SEARCH_WINDOW_REPLACE));
	parent->AddChild(replace);
	buttonRect.OffsetBy(0.0f, buttonHeight + 10.0f);

	BButton *replaceAll = new BButton(buttonRect,"replaceAll","Replace All",new BMessage(SearchWindowConstants::K_SEARCH_WINDOW_REPLACE_ALL));
	...
	m_caseSensitiveCheck = new BCheckBox(buttonRect,"caseSensitiveCheck","Case Sensitive",new BMessage(SearchWindowConstants::K_SEARCH_WINDOW_SETTINGS_CHANGED));
	...
	m_wrapAroundCheck = new BCheckBox(buttonRect,"wrapAroundCheck","Wrap Around",new BMessage(SearchWindowConstants::K_SEARCH_WINDOW_SETTINGS_CHANGED));	
	...
	m_searchBackwardsCheck = new BCheckBox(buttonRect,"searchBackwardsCheck","Search Backwards",new BMessage(SearchWindowConstants::K_SEARCH_WINDOW_SETTINGS_CHANGED));
	...
	m_allDocsCheck = new BCheckBox(buttonRect,"allDocsCheck","Replace In All",new BMessage(SearchWindowConstants::K_SEARCH_WINDOW_SETTINGS_CHANGED));
...
void SearchWindow::MessageReceived(BMessage *message)
{
	switch(message->what)
	{
		...
		{
			prefsLock.Lock();
			preferences.ReplaceBool(K_IS_ALL_DOCS, m_allDocsCheck->Value());
			preferences.ReplaceBool(K_IS_SEARCH_BACKWARDS, m_searchBackwardsCheck->Value());
			preferences.ReplaceBool(K_IS_WRAP_AROUND, m_wrapAroundCheck->Value());
			preferences.ReplaceBool(K_IS_CASE_SENSITIVE, m_caseSensitiveCheck->Value());			
			prefsLock.Unlock();
		}
		break;
...
void ParentView::Draw(BRect updateRect)
{
...
//==================================ScrollView=====================================================
ScrollView::ScrollView(const char* name, BView *target, uint32 rmode, uint32 flags, 
...
void ScrollView::MakeFocus(bool focus)
{

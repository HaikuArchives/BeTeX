...
class SearchWindow : public BWindow
{
	...
	BCheckBox	*m_caseSensitiveCheck,
			*m_wrapAroundCheck,
			*m_searchBackwardsCheck,
			*m_allDocsCheck;
};
...
{
	public:
	ScrollView(const char* name, BView* target, uint32 rmode = B_FOLLOW_LEFT|B_FOLLOW_TOP,	uint32 flags = 0,
		bool horizontal = false, bool vertical = false,	border_style border = B_FANCY_BORDER);
	virtual	~ScrollView();

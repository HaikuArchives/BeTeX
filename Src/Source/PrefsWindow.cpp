					...
					m_fontSizeChanged(false)
{
	...
	PrefsListView *prefsListView = new PrefsListView(prefsListFrame, m_parent, new BMessenger(this));
	...
	BRect boxRect(10.0 + prefsListWidth + 10.0 , 2.0, Bounds().right - 7.0 , Bounds().bottom - 10.0 - respiteWidth);
		...
		case PrefsConstants::K_PREFS_UPDATE:
		{
			...
			{ 
				...
				else if (is_instance_of(changedView, BCheckBox))
				{
					...
					bool val;
					if(	   prefsID == K_IS_GREEK_ABSENT 
						|| prefsID == K_IS_BIG_ABSENT
						|| prefsID == K_IS_BINARY_ABSENT
						|| prefsID == K_IS_MISC_ABSENT
						|| prefsID == K_IS_BIN_REL_ABSENT
						|| prefsID == K_IS_MMA_ABSENT
						|| prefsID == K_IS_INT_ABSENT
						|| prefsID == K_IS_TYPE_FACE_ABSENT
				...
				else if (is_instance_of(changedView, ColorButton))
				{
					...
					preferences_archive.ReplaceData(prefsID.String(),B_RGB_COLOR_TYPE, &new_color, sizeof(rgb_color));
				}
...
BView* PrefsWindow::constructGeneralBox(BRect frame)
{
	...
	BRect bubbleFrame = BRect(controlX, offset - adjust, offset + 150.0, offset + cbheight - adjust);

	ColorButton *bubbleColorButton = new ColorButton(bubbleFrame, K_BUBBLE_COLOR, GetPrefsMessage(K_BUBBLE_COLOR), preferences->bubble_color, "Tooltips");
...
BView* PrefsWindow::constructCommandBox(BRect frame)
{
	...
	BRect bitRect(offset / 2.0, offset + globalTcOffset, offset / 2.0 + 31.0, offset + 31.0 + globalTcOffset);
	BRect secCol(bitRect);
	secCol.OffsetBy(secOffset, 0.0);
	//secCol.OffsetBy(0.0, bitoffset);

	float	textCtrlWidth = 105.0,
			textCtrlHeight = 20.0,
			textCtrlCo = 6.0
			;

	BRect bitctrlRect(offset / 2.0 + bitoffset, offset + textCtrlCo + globalTcOffset, offset / 2.0 + bitoffset + textCtrlWidth, textCtrlCo + offset + textCtrlHeight + globalTcOffset);
...
BView* PrefsWindow::constructColorBox(BRect frame)
{
	...
	ColorButton *fgColorButton = new ColorButton(ccRect, K_FG_COLOR, GetPrefsMessage(K_FG_COLOR), preferences->fg_color," Text");
	colorBox->AddChild(fgColorButton);

	ccRect.OffsetBy(0,cbheight+cbspacing);
	ColorButton *bgColorButton = new ColorButton(ccRect, K_BG_COLOR, GetPrefsMessage(K_BG_COLOR), preferences->bg_color, "Background");
	colorBox->AddChild(bgColorButton);

	ccRect.OffsetBy(0,cbheight+cbspacing);
	ColorButton *genericCmdColorButton = new ColorButton(ccRect, K_GEN_CMD_COLOR, GetPrefsMessage(K_GEN_CMD_COLOR), preferences->generic_cmd_color, "Generic Commands");
	colorBox->AddChild(genericCmdColorButton);

	ccRect.OffsetBy(0,cbheight+cbspacing);
	ColorButton *formatCmdColorButton  = new ColorButton(ccRect, K_FORMAT_CMD_COLOR, GetPrefsMessage(K_FORMAT_CMD_COLOR), preferences->format_cmd_color, "Formatting Commands");
	colorBox->AddChild(formatCmdColorButton);

	ccRect.OffsetBy(0,cbheight+cbspacing);
	ColorButton *specialCmdColorButton = new ColorButton(ccRect, K_SPECIAL_CMD_COLOR, GetPrefsMessage(K_SPECIAL_CMD_COLOR), preferences->special_cmd_color, "Special Commands");
	colorBox->AddChild(specialCmdColorButton);

	ccRect.OffsetBy(0,cbheight+cbspacing);
	ColorButton *mathModeColorButton = new ColorButton(ccRect, K_MATH_MODE_COLOR, GetPrefsMessage(K_MATH_MODE_COLOR), preferences->math_mode_color, "Math Mode");
	colorBox->AddChild(mathModeColorButton);

	ccRect.OffsetBy(0,cbheight+cbspacing);
	ColorButton *commaColorButton = new ColorButton(ccRect, K_COMMA_COLOR, GetPrefsMessage(K_COMMA_COLOR), preferences->comma_color, "Quotation Marks");
	colorBox->AddChild(commaColorButton);

	ccRect.OffsetBy(0,cbheight+cbspacing);
	ColorButton *commentColorButton = new ColorButton(ccRect, K_COMMENT_COLOR, GetPrefsMessage(K_COMMENT_COLOR), preferences->comment_color,"Comments");
	colorBox->AddChild(commentColorButton);

	ccRect.OffsetBy(0,cbheight+cbspacing);
	ColorButton *puncSymbolColorButton = new ColorButton(ccRect, K_PUNC_SYMBOL_COLOR, GetPrefsMessage(K_PUNC_SYMBOL_COLOR), preferences->punc_symbol_color,"Punctuation / Symbols");
	colorBox->AddChild(puncSymbolColorButton);

	ccRect.OffsetBy(0,cbheight+cbspacing);
// TODO REMOVE URL	ColorButton *htmlColorButton = new ColorButton(ccRect, K_HTML_COLOR, GetPrefsMessage(K_HTML_COLOR), preferences->html_color,"Urls");
// TODO REMOVE URL	colorBox->AddChild(htmlColorButton);
	...
	BButton *resetColorDefaults = new BButton(cbbtnRect,"fResetColorDefaults","Defaults",new BMessage(PrefsConstants::K_PREFS_VIEW_RESET_COLOR_DEFAULTS));
	colorBox->AddChild(resetColorDefaults);

	float	sliderWidth = 300.0,
			vertOffset = 40.0,
			minVal = 5.0,
			maxVal = 30.0
			;
	BRect sliderFrame(ccRect.left, ccRect.top + vertOffset, ccRect.left + sliderWidth, ccRect.top + vertOffset + 100.0);
...
BView* PrefsWindow::constructToolbarBox(BRect frame)
{
	...
	BRect checkboxRect = BRect((offset * 2.0 / 3.0), offset - adjust,(offset * 2.0/ 3.0) + 150.0, offset - adjust + vertCbSpacing - 5.0);

	BCheckBox *tbInvertCheck = new BCheckBox(checkboxRect, K_IS_TEXBAR_INVERTED,"Invert Toolbar Colors", GetPrefsMessage(K_IS_TEXBAR_INVERTED));
	...
	BCheckBox *tbGreekAbsentCheck = new BCheckBox(checkboxRect, K_IS_GREEK_ABSENT,"Greek Letters", GetPrefsMessage(K_IS_GREEK_ABSENT));
	...
	BCheckBox *tbBigAbsentCheck = new BCheckBox(checkboxRect, K_IS_BIG_ABSENT,"Big Operators", GetPrefsMessage(K_IS_BIG_ABSENT));
	bool isBigAbsent = preferences->IsBigAbsent;
	tbBigAbsentCheck->SetValue(!isBigAbsent);
	toolbarBox->AddChild(tbBigAbsentCheck);
	checkboxRect.OffsetBy(0, vertCbSpacing);

	BCheckBox *tbBinaryAbsentCheck = new BCheckBox(checkboxRect, K_IS_BINARY_ABSENT,"Binary Operators", GetPrefsMessage(K_IS_BINARY_ABSENT));
	...
	BCheckBox *tbMiscAbsentCheck = new BCheckBox(checkboxRect, K_IS_MISC_ABSENT,"Misc Symbols", GetPrefsMessage(K_IS_MISC_ABSENT));
	bool isMiscAbsent = preferences->IsMiscAbsent;
	tbMiscAbsentCheck->SetValue(!isMiscAbsent);
	toolbarBox->AddChild(tbMiscAbsentCheck);
	checkboxRect.OffsetBy(0, vertCbSpacing);

	BCheckBox *tbBinRelAbsentCheck = new BCheckBox(checkboxRect, K_IS_BIN_REL_ABSENT,"Binary Relations", GetPrefsMessage(K_IS_BIN_REL_ABSENT));
	...
	BCheckBox *tbMMAAbsentCheck = new BCheckBox(checkboxRect, K_IS_MMA_ABSENT,"Math Mode Accents", GetPrefsMessage(K_IS_MMA_ABSENT));
	bool isMMAAbsent = preferences->IsMMAAbsent;
	tbMMAAbsentCheck->SetValue(!isMMAAbsent);
	toolbarBox->AddChild(tbMMAAbsentCheck);
	checkboxRect.OffsetBy(0, vertCbSpacing);

	BCheckBox *tbIntlAbsentCheck = new BCheckBox(checkboxRect, K_IS_INT_ABSENT,"International", GetPrefsMessage(K_IS_INT_ABSENT));
	bool isIntlAbsent = preferences->IsIntlAbsent;
	tbIntlAbsentCheck->SetValue(!isIntlAbsent);
	toolbarBox->AddChild(tbIntlAbsentCheck);
	checkboxRect.OffsetBy(0, vertCbSpacing);

	BCheckBox *tbTypeFaceAbsentCheck = new BCheckBox(checkboxRect, K_IS_TYPE_FACE_ABSENT,"Type Faces", GetPrefsMessage(K_IS_TYPE_FACE_ABSENT));
	...
	BCheckBox *tbFunctionAbsentCheck = new BCheckBox(checkboxRect, K_IS_FUNCTION_ABSENT,"Functions", GetPrefsMessage(K_IS_FUNCTION_ABSENT));

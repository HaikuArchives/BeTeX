...
void TexView::Undo(BClipboard* clip)
{	
	BTextView::Undo(clip);
	DontSel = false;
	IsShifting = false;	
...
void TexView::Select(int32 start,int32 finish)
{
...
void TexView::SetNormalFocus()
{
	IsNormalFocus = true;
	
}

void TexView::SetAttachedFocus()
{
	HasAttachedFocus = true;
}

void TexView::MouseUp(BPoint point)
{
	IsColSel = false;
	BTextView::MouseUp(point);
}

void TexView::Draw(BRect r)
{
	BTextView::Draw(r);
}

void TexView::MouseMoved(BPoint point,uint32 transit,const BMessage* msg)
{
	if(!IsNormalFocus)
	{
		switch(transit)
...
int32 TexView::CountPhysicalLines()
{
	return BTextView::CountLines();
}

void TexView::MouseDown(BPoint point)
{
	BTextView::MouseDown(point);
}
void TexView::KeyDown(const char* bytes, int32 numBytes)
{
	switch(bytes[0])
	{
		case B_ENTER:
		{
			...
			int32 start,finish;
			GetSelection(&start,&finish);
			if(msg.AddInt32("start",start) == B_OK
			&& msg.AddInt32("finish",finish) == B_OK)
...
int TexView::FindFirstOnLine(char c,int offset,int eol)
{
...
int32 TexView::OffsetAtIndex(int32 index)
{
	//int text_length = TextLength();
	int i;
	int count = 0;
...
int32 TexView::OffsetAt(BPoint point)
{
...
void TexView::ParseLine(int sol,int eol,vector<rgb_color>& colorVec)
{
		int i;
		int offset = sol;
		int pos;
...
void TexView::TwoColorPlateau(char c,int sol,int eol,rgb_color c1,vector<rgb_color>& colorVec)
{
		int IsPlateau = false;
		int plLength = 0;
		int plStart;
		...
		for(i=sol;i<eol;i++)
		{
			...
			else if(c != '\\' && !IsPlateau && ByteAt(i) == c &&  i - 1 >= 0 && ByteAt(i-1) != '\\')
			{
				plStart = i;
				plLength++;
				IsPlateau = true;
...
void TexView::ITwoColorPlateau(char c,int sol,int eol,rgb_color c1,vector<rgb_color>& colorVec)
{
		int IsPlateau = false;
		int plLength = 0;
		int plStart;
...
void TexView::SetMathModes(vector<rgb_color>& colorVec)
{
...
void TexView::InsertText(const char* text,int32 length,int32 offset,const text_run_array* runner)
{
	...
	if(!IsShifting && text[0] != B_ENTER)
	{
		...
		if(msg.AddInt32("offset",offset)==B_OK
		&& msg.AddInt32("length",length)==B_OK)
		Window()->PostMessage(&msg,this);		
	}
...
void TexView::DeleteText(int32 start, int32 finish)
{	
	if(!IsShifting)
	{
		BMessage msg(InterfaceConstants::K_ANALYSE_TEXVIEW);
...
bool TexView::Contains(vector<BString>& v,BString s)
{
	for(int i=0;i<v.size();i++)
	{
		if(v[i] == s)
...
bool TexView::CanEndLine(int32 offset)
{
...
int32 TexView::GetMathSol(int sol)
{
	/*bool IsMultiLine;
	vector<BString> start;
	start.push_back("$");
...
void TexView::LoadFile (entry_ref *ref)
{
	if (ref == NULL) 
	{
		return;

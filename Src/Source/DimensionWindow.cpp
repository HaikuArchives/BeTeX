...
DimensionWindow::DimensionWindow(BRect frame, BMessenger* messenger, BString standardWidth, BString standardHeight, uint32 return_msg) 
					:	BWindow(frame,"Dimensions", B_FLOATING_WINDOW, B_NOT_ZOOMABLE|B_NOT_RESIZABLE|B_ASYNCHRONOUS_CONTROLS|B_NOT_CLOSABLE),
						...
						m_ownerMsgr(messenger)
{
	...
	BRect buttonRect(Bounds().left + (2.0f * buttonPad) - 5.0f, Bounds().bottom - (2.0f * buttonPad) - buttonHeight, Bounds().left + (2*buttonPad) + buttonWidth - 5, Bounds().bottom - (2.0f * buttonPad));	
	BButton *okButton = new BButton(buttonRect, "Ok", "Ok", new BMessage(InterfaceConstants::K_DIM_WINDOW_OK));	
	parent->AddChild(okButton);	
	okButton->MakeDefault(true);
	
	buttonRect.OffsetBy((50.0f + (2.0f * buttonPad)),0.0f);
	BButton *cancelButton = new BButton(buttonRect, "Cancel", "Cancel", new BMessage(InterfaceConstants::K_DIM_WINDOW_CANCEL));
...
void DimensionWindow::MessageReceived(BMessage* message)
{
	switch(message->what)
	{		

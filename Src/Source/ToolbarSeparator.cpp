...
status_t
WToolbarSeparator::Archive(BMessage* archive, bool deep) const
{
...
BArchivable*
WToolbarSeparator::Instantiate(BMessage* archive)
{
	return (validate_instantiation(archive, "WToolbarSeparator") ? new WToolbarSeparator(archive)
																 : NULL);
}
...
void
WToolbarSeparator::Draw(BView* canvas, BRect updateRect)
{
	if (Toolbar() == NULL)
		return;
	switch (Toolbar()->Style()) {
		...
		{
			float l, r, t, b, p;
			p = ((Toolbar()->Alignment() == W_TOOLBAR_VERTICAL ? Bounds().Height()
															   : Bounds().Width())
					+ 1)
					/ 2.0
				- 1.0;
...
void
WToolbarSeparator::GetPreferredSize(float* width, float* height)
{
	float w, h, s = 0, o = 0;
	if (Toolbar() != NULL) {
		switch (Toolbar()->Style()) {

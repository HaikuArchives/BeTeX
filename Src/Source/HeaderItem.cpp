...
void HeaderItem::ScrollBy(float h,float v)
{
	BView::ScrollBy(h,v);
	Invalidate();
}

void HeaderItem::ScrollTo(BPoint where)
{
...
void HeaderItem::AttachedToWindow()
{
	BView::AttachedToWindow();
}

void HeaderItem::MessageReceived(BMessage* msg)
{
	switch(msg->what)
	{
		//case MENU_FILE_OPEN:
...
void HeaderItem::MouseDown(BPoint point)
{
		uint32 button;
...
void HeaderItem::MouseUp(BPoint point)
{
		IsDown = false;
		Invalidate();
...
void HeaderItem::SetHidden(bool tohideornottohide)
{
...
void HeaderItem::FrameResized(float w, float h)
{
	Invalidate();

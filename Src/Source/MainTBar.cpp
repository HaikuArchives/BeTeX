...
void ChurnBitmap(BBitmap* bitmap)
{	//B G R A
	...
	{	
		...
		//cout << "RED = " << (int)p[i+2] << ", GREEN = " << (int)p[i+1] << ", BLUE = " << (int)p[i] << ", ALPHA = "<< (int)p[i+3] << endl;
	}
}


MainTBar::MainTBar(BRect r,BubbleHelper* h) : BView(r,"maintbar",B_FOLLOW_NONE,B_WILL_DRAW)
{
	helper = h;
	...
	BRect tiler = BRect(1,1,offset,offset);
			...
			AddChild(TBNewDoc = new TButton(tiler,new BMessage(K_MENU_FILE_NEW),NewDocBitmap));
			...
			AddChild(TBOpen = new TButton(tiler,new BMessage(K_MENU_FILE_OPEN),OpenBitmap));
			...
			AddChild(TBSave = new TButton(tiler,new BMessage(K_MENU_FILE_SAVE),SaveBitmap));
			...
			AddChild(TBPrint = new TButton(tiler,new BMessage(K_MENU_FILE_PRINT),PrinterBitmap));
			helper->SetHelp(TBPrint,"Print");
			tiler.OffsetBy(offset,0);
			
/*			BBitmap* DelTmpBitmap = new BBitmap(BRect(0,0,31,31),B_CMAP8);
			DelTmpBitmap->SetBits(DelTmp,3072,0,B_CMAP8);
			AddChild(TBDelTmp = new TButton(tiler,new BMessage(CMD_DELETE_TEMP_FILES),DelTmpBitmap));
			helper->SetHelp(TBDelTmp,"Delete Temporary Files");
			tiler.OffsetBy(offset,0);
*/			
/*			BBitmap* EmptyDelTmpBitmap = new BBitmap(BRect(0,0,31,31),B_CMAP8);
			EmptyDelTmpBitmap->SetBits(EmptyTmp,3072,0,B_CMAP8);
...
*/			
			...
			AddChild(TBViewLog = new TButton(tiler,new BMessage(K_CMD_VIEW_LOG_FILE),ViewLogBitmap));
			...
			AddChild(TBOpenTracker = new TButton(tiler,new BMessage(K_CMD_LAUNCH_TRACKER),TrackerBitmap));
			...
			AddChild(TBOpenTerminal = new TButton(tiler,new BMessage(K_CMD_LAUNCH_TERMINAL),TerminalBitmap));
			...
			AddChild(TBTexDvi = new TButton(tiler,new BMessage(K_CMD_COMPILE_TEXDVI),TexDviBitmap));
			...
			AddChild(TBDviPdf = new TButton(tiler,new BMessage(K_CMD_COMPILE_DVIPDF),DviPdfBitmap));
			...
			AddChild(TBDviPs = new TButton(tiler,new BMessage(K_CMD_COMPILE_DVIPS),DviPsBitmap));
			...
			AddChild(TBPsPdf = new TButton(tiler,new BMessage(K_CMD_COMPILE_PSPDF),PsPdfBitmap));
			...
			AddChild(TBTexPdf = new TButton(tiler,new BMessage(K_CMD_COMPILE_TEXPDF),TexPdfBitmap));
			...
			AddChild(TBTexHtml = new TButton(tiler,new BMessage(K_CMD_COMPILE_TEXHTML),TexHtmlBitmap));
			...
			PrevDVIBitmap->SetBits(PrevDVI,3072,0,B_CMAP8);
			AddChild(TBPrevDVI = new TButton(tiler,new BMessage(CMD_LAUNCH_DVIVIEWER),PrevDVIBitmap));
			...
			*/
			...
			AddChild(TBPrevPS = new TButton(tiler,new BMessage(K_CMD_LAUNCH_PSVIEWER),PrevPSBitmap));
			...
			AddChild(TBPrevPDF = new TButton(tiler,new BMessage(K_CMD_LAUNCH_BE_PDF),PrevPDFBitmap));

...
void ProjectView::SetSplitPane(SplitPane* owner)
{
	m_ownerSplitPane = owner;
}

void ProjectView::MouseDown(BPoint point)
{
	GetMouse(&point, &m_button);
	BListView::MouseDown(point);
}

void ProjectView::Next(int32 increment)
{
...
void ProjectView::Prev(int32 decrement)
{
	int32 items = CountItems();
	if(items > 1)
	{
...
void ProjectView::KeyDown(const char* bytes,int32 numBytes)
{
	switch(bytes[0])
	{
		case B_HOME:
...
int32 ProjectView::NoVisibleItems()
{
	BRect bounds = Bounds();
	int32 noitems = 0;

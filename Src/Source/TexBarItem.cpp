...
TexBarItemView::TexBarItemView(BRect f,const char* n,uint32 rm,uint32 fl) : BView(f,n,rm,fl)
{
}

void TexBarItemView::MouseMoved(BPoint point,uint32 transit,const BMessage* msg)
{
	/*switch(transit)
	{
		case B_ENTERED_VIEW:
...
TexBarItem::TexBarItem(BRect r,const char* l,TexBarItemView* p) : BView(r,"TexBarItem",B_FOLLOW_TOP,B_WILL_DRAW)
{
...
float TexBarItem::Height()
{
	return IsHidden ? ItemHeight : PlateauHeight + ItemHeight + 1;
}

void TexBarItem::ScrollBy(float h,float v)
{
	BView::ScrollBy(h,v);
	//Invalidate();
}

void TexBarItem::ScrollTo(BPoint where)
{
	BView::ScrollTo(where);
	//Invalidate();
}

void TexBarItem::Draw(BRect r)
{
	r = Bounds();	
...
void TexBarItem::AttachedToWindow()
{
	//SetViewColor(0,0,255);
	BView::AttachedToWindow();
}

void TexBarItem::MessageReceived(BMessage* msg)
{
	switch(msg->what)
	{
		//case MENU_FILE_OPEN:
...
void TexBarItem::MouseDown(BPoint point)
{
...
void TexBarItem::MouseUp(BPoint point)
{
	//uint32 button;
	//GetMouse(&point,&button);
	//if(button == B_PRIMARY_MOUSE_BUTTON)
...
void TexBarItem::MouseMoved(BPoint point,uint32 transit,const BMessage* msg)
{

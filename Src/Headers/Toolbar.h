...
private:
	...
	void _draw_item(unsigned line, unsigned position, BRect updateRect);
...
public:
	WToolbar(BRect frame, const char* name, int style = W_TOOLBAR_STYLE_FLAT,
		WToolbarAlignment alignment = W_TOOLBAR_HORIZONTAL, bool auto_size = true,
		border_style border = B_FANCY_BORDER, uint32 resizing_mode = B_FOLLOW_LEFT | B_FOLLOW_TOP,
		uint32 flags = B_FRAME_EVENTS | B_FULL_UPDATE_ON_RESIZE | B_WILL_DRAW);
	...
	// Items management
	bool AddItem(
		WToolbarItem* item, int line = W_TOOLBAR_LAST_LINE, int position = W_TOOLBAR_LAST_POSITION);
	unsigned CountItems(bool visibleOnly = false);
	unsigned CountItemsInLine(unsigned line, bool visibleOnly = false);
	...
	WToolbarItem* ItemAt(unsigned line, unsigned position, bool visibleOnly = false);
	bool LineEnabled(unsigned line);
	bool LineVisible(unsigned line);
	bool MoveItem(WToolbarItem* item, int line, int position = W_TOOLBAR_LAST_POSITION);
	...
	void SetAlignment(WToolbarAlignment alignment);
	void SetBackgroundBitmap(BBitmap* bitmap);
	void SetBorder(border_style border);
	void SetLabelPosition(WToolbarLabelPosition label_position);
	...
	status_t SetControlsTarget(const BHandler* handler, const BLooper* looper = NULL);

...
TListItem::TListItem(SplitPane* sp,BScrollView* docScroll,BString label,Prefs* p) : BListItem()
{
	...
	sv = new BScrollView("ScrollView2",tv,B_FOLLOW_LEFT | B_FOLLOW_TOP,B_WILL_DRAW|B_FRAME_EVENTS,false,true,B_FANCY_BORDER);
...
TListItem::TListItem(SplitPane* sp,BScrollView* docScroll,entry_ref* r,Prefs* p,bool IsTemplate=false)
{
	prefs = p;
	if(!IsTemplate)
		ref = *r;
...
entry_ref TListItem::GetRef()
{
	return ref;
}
void TListItem::DrawItem(BView* owner,BRect frame,bool complete=false)
{
	rgb_color color;
	...
	if(IsSelected() || complete)
	{
		...
		{ 
			...
			{	
				...
				color = owner->ViewColor();//tint_color(unsaved_color,B_LIGHTEN_2_TINT);//ui_color(B_WINDOW_TAB_COLOR);	
			}
			else
...
void TListItem::SetRef(entry_ref* r)
{
	ref = *r;
	BPath path(&ref);
	fname = "";
...
const char* TListItem::FName()
{
	return fname.String();
}
void TListItem::SetFName(BString name)
{
	fname = name;
}
void TListItem::ShowTextView()
{
...
void TListItem::HideTextView()
{
	SP->AddChildTwo(sv,true,false);
	sv->ScrollBar(B_VERTICAL)->SetValue(0);
	
}
TexView* TListItem::TextView()
{
	return tv;
}

bool TListItem::IsSaveNeeded()
{
	return NeedToSave;

}
void TListItem::SetSaveNeeded(bool flag)
{
	NeedToSave = flag;
}
bool TListItem::IsHomely()
{
	return HasHome;

}
void TListItem::SetHomely(bool flag)
{
	HasHome = flag;
}

...
void FastColor::SetValue(int32 v)
{
...
void FastColor::SetValue(rgb_color color)
{
	BColorControl::SetValue(color);
}
//============================================================================
ColorWindow::ColorWindow(BRect frame, BMessenger* messenger, rgb_color color) 
				:	BWindow(frame, "Pick a Color" B_UTF8_ELLIPSIS, B_FLOATING_WINDOW, B_NOT_ZOOMABLE|B_NOT_RESIZABLE|B_ASYNCHRONOUS_CONTROLS|B_NOT_CLOSABLE),
					m_msgr(messenger)
{
	...
	m_ctrlBox = new FastColor(m_colorView, BPoint(10,10), B_CELLS_32x8, 4.0f, "ctrlbox", new BMessage(ColorPrefsConstants::K_COLOR_PREFS_UPDATE), true);
	...
	m_okButton = new BButton(btnRect,"fOk","Ok",new BMessage(ColorPrefsConstants::K_COLOR_PREFS_OK));
	m_parent->AddChild(m_okButton);
	m_okButton->MakeDefault(true);

	btnRect.OffsetBy(-offset,0);
	m_cancelButton = new BButton(btnRect,"fCancel","Cancel",new BMessage(ColorPrefsConstants::K_COLOR_PREFS_CANCEL));
